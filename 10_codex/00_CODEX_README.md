...
1) `10_codex/TEMPLATE_MANIFEST.yaml` 是模板结构的 Source of Truth，列出必须保留的目录和文件（`00_human` 五件套、`0-source/raw/` + `parsed/` + `registry.yaml`、`1_story/script_breakdown_v1.yaml`、`2_layout/`、`3_prompt/`、`4_render/` 等）。每当引入新路径，应同步在该清单添加新条目，并让 `preflight` (see below) 触发之。
