...
1) `10_codex/TEMPLATE_MANIFEST.yaml` 是模板结构的 Source of Truth，列出必须保留的目录和 files（`00_human` 五件套、`30_project/inputs/script/source_script.md`、`0-source/raw/` + `parsed/` + `registry.yaml`、`1_story/script_breakdown_v1.yaml`、`2_layout/`、`2_audio/`、`5_color/`、`2_layout/2-2_layout_freeze.yaml` 等）；`20_runtime/` 仅作为 cache 运行时骨架（plan/exec/tests/final），内含 `.gitkeep`，可随时清空而不影响模板。每当引入新路径或 archive 旧路径，应同步在该清单添加新条目，并让 `preflight` (see below) 触发之。
